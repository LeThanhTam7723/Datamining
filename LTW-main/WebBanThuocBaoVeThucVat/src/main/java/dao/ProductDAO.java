...
public class ProductDAO implements IProductDAO {
    ...
    public List<Set<Integer>> transactions (){
        Jdbi jdbi = JDBIConnector.getJdbi();
        String sql = "SELECT GROUP_CONCAT(id_product ORDER BY id_product ASC SEPARATOR ', ') AS products\n" +
                "FROM order_details\n" +
                "GROUP BY id_order\n" +
                "ORDER BY id_order;";
        return jdbi.withHandle(handle ->
                handle.createQuery(sql)
                        .mapTo(String.class)
                        .map(productsStr -> {
                            Set<Integer> productSet = Arrays.stream(productsStr.split(","))
                                    .map(String::trim)  // Loại bỏ khoảng trắng dư thừa
    ...
    public List<Products> getThePopular(List<Integer> transactions) {
        Jdbi jdbi = JDBIConnector.getJdbi();

        // Nếu danh sách rỗng, trả về danh sách trống
        ...
        return jdbi.withHandle(handle -> {
            String sql = "SELECT id, product_name, image, price, id_category, status, des, create_at, inventory_quantity " +
                    "FROM products " +
                    "WHERE id IN (<ids>) " +  // Sử dụng Named Parameter
                    "LIMIT 4";

            return handle.createQuery(sql)
                    .bindList("ids", transactions) // Truyền danh sách ID
                    .mapToBean(Products.class)
                    .list();
        });
    }
