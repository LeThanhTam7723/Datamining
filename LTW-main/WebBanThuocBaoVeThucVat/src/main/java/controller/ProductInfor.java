...
import Apirori.AprioriAlgorithm;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (id != null && !id.isEmpty()) {
            try {
                ...
                //dwưới là phần sản phẩm được bán kèm.
                ProductDAO productDAO = new ProductDAO();
                List<Set<Integer>> transactions = productDAO.transactions();
                AprioriAlgorithm algorithm = new AprioriAlgorithm();
                List<Integer> topItemsWithA = algorithm.findFrequentItemsWithA(transactions, Integer.parseInt(id));
                List<Products> topItems = productDAO.getThePopular(topItemsWithA);// danh sách sản phẩm đc mua kèm nhiều nhất
                request.setAttribute("proBuyWith", topItems);
